...
class Config:
    ...
    # CORS Configuration
    # Allow both ports 3000 and 3001 (frontend dev server import os
from datetime import timedelta
...
class TestingConfig(Config):
    TESTING = True
    MONGO_URI = os.environ.get('MONGO_URI') or 'mongodb+srv://anushjadhav:anush3108@ecommerce.ppn25ky.mongodb.net/ 'may run on either)
    CORS_ORIGINS = [
        'http://localhost:3000',
        'http://127.0.0.1:3000',
        'http://localhost:3001',
...
class ProductionConfig(Config):
    DEBUG = False
    MONGO_URI = os.environ.get('MONGO_URI', 'mongodb://localhost:27017/ecommerce_prod')
...
class TestingConfig(Config):
    TESTING = True
    MONGO_URI = os.environ.get('MONGO_URI') or 'mongodb://localhost:27017/ecommerce_test'
